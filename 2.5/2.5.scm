...
(define (install-polynomial-package)
    ...
    (define (zero? p) (= (variable p) 0))
    

    ;⟨2.3.2 節の same-variable? と variable? ⼿続き⟩
    ;; 項と項リストの表現
    ;⟨下記の adjoin-term . . . coeff ⼿続き⟩
    (define (add-poly p1 p2) p1)
    ;    ⟨add-poly が使う⼿続き⟩
    (define (mul-poly p1 p2) p1)
    ;   ⟨mul-poly が使う⼿続き⟩
...
(print "===Ex2.87===")
(define p0 (make-polynomial 0 'x))
(define p1 (make-polynomial 3 'x))
(define p2 (make-polynomial 5 'x))

(define (ex-2-87)
    (print "p0: " p0 " zero?="(=zero? p0))
    (print "p1: " p1 " zero?="(=zero? p1))
)
(ex-2-87)
